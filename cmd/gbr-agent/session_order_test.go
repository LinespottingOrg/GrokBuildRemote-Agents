...
func TestPrioritizeSessionIDs_GrokFirstNoCap(t *testing.T) {
	...
	if len(got) != 8 {
		t.Fatalf("len=%d want all 8 (under 255): %v", len(got), got)
	}
...
func TestPrioritizeSessionIDs_SoftMax255KeepsGrok(t *testing.T) {
	in := make([]string, 0, 300)
	for i := 0; i < 280; i++ {
		in = append(in, "conhost-"+itoaPID(uint32(i)))
	}
	in = append(in, "grok-build-40a22", "gbr-agent")
	got := prioritizeSessionIDs(in)
	if len(got) != session.MaxSessions {
		t.Fatalf("len=%d want soft max %d", len(got), session.MaxSessions)
	}
	if got[0] != "grok-build-40a22" {
		t.Fatalf("first=%q — Grok must survive the 255 clip", got[0])
	}
	if got[1] != "gbr-agent" {
		t.Fatalf("second=%q want gbr-agent", got[1])
	}
