...
func TestPairDeepLinkIdentDoesNotLeakKey(t *testing.T) {
	...
	for _, need := range []string{"code=A1B2C3D4", "ver=0.6.2", "sha256=aaaaaaaa", "os=darwin", "keyfp=aabbccddeeff", "host=mac-mini"} {
...
func TestPairPageURLCarriesIdentity(t *testing.T) {
	id := pairIdent{Code: "ABCD2345", Version: "0.6.2", SHA256: "abc", KeyFP: "deadbeefcafe", MailboxID: "gbr-abcd2345"}
	u := pairPageURL(id)
