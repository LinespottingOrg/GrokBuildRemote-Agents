...
There is **no session fan-out cap**. Every discovered window is advertised. Grok Build is sorted first, then the agent shell, then other terminals.
...
Agent 0.5.0 sent periodic feedback for only the **first six** sessions, in map order. Extra PowerShell windows filled those slots; Grok was dropped. This branch:
