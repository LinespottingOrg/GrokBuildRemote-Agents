...
It does NOT include Agents PR #40 (ack-on-fail / same command_id / GBR_INJECT_HALT / -inject-halt).
Replace $Exe with a rebuild from origin/main AFTER PR #40 (merge f7bd6c1, 2026-09-01).
...
function Get-AgentRunArguments {
  ...
  $parts.Add("-log=info")
  $parts.Add("run")
  if ($HaltInject) { $parts.Add("-inject-halt") }
  $parts.Add("-no-auto-open")
  if ($HelpText -match 'no-inbox-watch') { $parts.Add("-no-inbox-watch") }
  return ($parts -join " ")
}
...
Assert-AgentHasPR40Guards -Exe $exe
