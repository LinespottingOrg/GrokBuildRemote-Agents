...
| Dispatched to a remote but it ran locally | Unknown device names **silently fall back to `local`** | gbr-mcp sets `_warning` on the response. Run `gbr_devices`; register with `gbr-agent fleet add` |
