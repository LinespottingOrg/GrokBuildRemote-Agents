...
Requires **gbr-agent ≥ 0.5.3** for inject/status, **≥ 0.5.4** for the open → lock →
idle-wait → result loop (same contract as a Grok bot). Run locally, or a relay
mailbox + key for remote mode.
...
| `gbr_open` | Spawn `grok` / `grok --resume` or attach a live session, then take a lease |
| `gbr_result` | Wait for idle (prompt or quiet) and harvest an excerpt — the feedback loop |
| `gbr_lock` | Exclusive session lease so Grok bot and Claude Cowork do not share a window |
| `gbr_tasks` | Durable `~/.gbr/tasks.json` row for iterate / done / failed |
