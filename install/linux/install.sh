#!/usr/bin/env bash
# Grok Build Remote — Linux install
set -euo pipefail
...
if ! command -v xdotool >/dev/null 2>&1; then
  echo "note: xdotool not installed (optional for X11 UI inject). Managed shell still works."
  echo "      sudo apt install xdotool   # or: sudo dnf install xdotool"
fi
...
echo "  gbr-agent service install   # systemd --user"
