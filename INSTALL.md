...
**macOS non-interactive LaunchAgent (Background, no Interactive ProcessType):** on the Mac Mini, after a halt-capable `gbr-agent` is at `~/.local/bin/gbr-agent`, use [`scripts/darwin/`](scripts/darwin/README.md) (`install-service.sh`). Args: `-log=info run -inject-halt`. Defaults: `GBR_INJECT_HALT=1`, `GBR_NO_AUTO_OPEN=1`, `GBR_INBOX_WATCH=0`, WorkingDirectory + `GBR_OPEN_CWD=$HOME/Developer`, logs `$HOME/Developer/gbr-agent-out`. Never `~/pc-build` (inbox #119). Label `com.linespotting.grok-build-remote`. Disable legacy `com.linespotting.gbr-agent` (do not delete without David yes). Login Items show **Grok Build Remote**. Do not run this installer from Windows.
