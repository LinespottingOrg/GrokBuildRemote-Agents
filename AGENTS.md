...
**Attach (OpenClaw / Hermes / NemoClaw):** one surface — Bot API `:8788` or `gbr-mcp`. Skill: [skills/openclaw/SKILL.md](skills/openclaw/SKILL.md). Pair stays `gbr-agent pair` (QR + 8-char) then `run`. Inbox comments (`gh`, label `boss-steer`) inject when a Grok Build title matches the issue title (`GBR_INBOX_WATCH=0` to disable). **Password managers** (`op` / `lpass`) are host CLIs after that pair, not a second protocol: [docs/PASSWORD-MANAGERS.md](docs/PASSWORD-MANAGERS.md) · [skills/password-managers/SKILL.md](skills/password-managers/SKILL.md).
...
| [docs/PASSWORD-MANAGERS.md](docs/PASSWORD-MANAGERS.md) | 1Password / LastPass host CLI — not a pair |
